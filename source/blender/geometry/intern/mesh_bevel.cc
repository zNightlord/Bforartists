...
/* Constructed vertex, sometimes later instantiated as mesh vertex. */
struct NewVert {
  int v;
  float3 co;
};
...
struct EdgeHalf {
  /** Index of mesh edge. */
  int edge;
  /** Index of mesh face between this edge and previous, if any. */
  int face_prev;
  /** Index of mesh face between this edge and next, if any. */
  int face_next;
  /** Position in BevVert's boundary of left boundary vert (looking along edge to end).
   * If this edge is beveled, the right boundary vert will be in the next cyclic position. */
  int boundary_vert_pos;
  /** Offset into profile to attach non-beveled edge. */
  int profile_index;
  /** Is this edge beveled? */
  bool is_bev;
  /** Is the second vertex of edge `edge` the vertex at this end? */
  bool is_rev;
  /** Is e a seam for custom loop-data (e.g., UVs). */
  bool is_seam;

  /** Initialize, assuming edge, is_bev, face_prev, face_next,  and is_rev are already set
   * properly. */
  void init(int vert_pos)
  {
    boundary_vert_pos = vert_pos;
    profile_index = 0;
    is_seam = false;
  }
};
...
struct BoundaryVert {
  NewVert new_vert;
  /** Position of the first edge in bevvert's edge_halfs of the first edge attached here.  */
  int edgehalf_first_pos;
  /** Count of how many edges total are attached here (CCW from first one). */
  int edgehalf_count;
  /** Position of the "edge between" that this boundvert on, in offset_on_edge_between case. -1 if
   * none. */
  int edgehalf_on_pos;
  /** Position of the beveled edge whose left side is attached here, if any.  -1 if none. */
  int edgehalf_bev_pos;
  /** When eon set, ratio of sines of angles to eon edge. */
  float sinratio;
  /** Adjustment chain or cycle link pointer. */
  int boundary_vert_adjchain;
  /** Edge profile between this and next BoundaryVert. */
  Profile profile;
  /** Length of seam starting from current BoundaryVert to next BoundaryVert with CCW ordering. */
  int seam_len;
  ...
  void init(int first_pos, int count = 1, int bev_pos = -1)
  {
    new_vert.v = -1;
    edgehalf_first_pos = first_pos;
    edgehalf_count = count;
    edgehalf_on_pos = -1;
    edgehalf_bev_pos = bev_pos;
    sinratio = 0.0f;
    boundary_vert_adjchain = -1;
    seam_len = 0;
    sharp_len = 0;
    any_seam = false;
    is_arc_start = false;
    is_patch_start = false;
    is_profile_start = false;
  }
};
...
struct VMesh {
  ...
  NewVert *mesh;
  /** Range in state's boundary_verts of this VMesh's boundary.. */
  IndexRange boundary_verts;
  /** The kind of mesh to build at the corner vertex meshes. */
  MeshKind mesh_kind;

  int verts_num() const
  {
    return boundary_verts.size();
  }
};

/* Data for a vertex involved in a bevel. */
struct BevVert {
  /** Range of state's edge_halfs: non-wire ones, sorted; CCW order from vertex normal side. */
  IndexRange edge_halfs_range;
  /** Range of state's edge_halfs that are wire edges (don't participate in bevel).. */
  IndexRange wire_edges_range;
  /** Original mesh vertex index. */
  int v;
  /** Number of beveled edges around the vertex. */
  int bev_num;
  ...
  int edges_num() const
  {
    return edge_halfs_range.size();
  }

  int edge_half_index(int edge_pos) const
  {
    return this->edge_halfs_range[edge_pos];
  }
};
...
class BevelState {
 public:
  ...
  /** Input Mesh. */
  const Mesh &src_mesh;
  /** Indices of mesh edges being beveled (edges attached to a beveled vertex  if a vertx-only
   * bevel). */
  IndexMask bevel_edges;
  /** Indices of mesh vertices at ends of beveled edges, (or to-bevel verts if vertex-only bevel.
   */
  IndexMask bevel_verts;
  ...
  bool offset_adjust;
  /** Beveled vertex state for each vertex in bevel_verts. */
  Array<BevVert> bevverts;
  /** Map from input vertex index to bevverts index. */
  Array<int> vert_to_bevvert;
  /** All EdgeHaffs needed, sorted so that the ranges needed for each BevVert are contiguous. */
  Array<EdgeHalf> edge_halfs;
  /** Left and right edge specs for each edge_half.
   ...
   */
  Array<float2> edge_half_specs;
  /** VMeshes needed for each vertex in bevel_verts. */
  Array<VMesh> vmeshes;
  /** All BoundaryVerts needed for doing the bevel.  */
  Array<BoundaryVert> boundary_verts;
  /** For each Mesh vert v, vert_to_edge[v] is a Span of edge indices incident on v. */
  GroupedSpan<int> vert_to_edge;
  /** For each Mesh edge e, edge_to_face[e] is a Span of face indices incident on e. */
  GroupedSpan<int> edge_to_face;

 private:
  /** Memory allocator for IndexMasks. */
  IndexMaskMemory memory_;
  /** Storage for vert_to_edge map. */
 ...
 public:
  ...
  void build_bevverts_and_halfedges();
  ...
  {
    return this->edge_halfs[bevvert.edge_halfs_range[edge_pos]];
  }

  /** Const version of previous function. */
  const EdgeHalf &edge_half(const BevVert &bevvert, const int edge_pos) const
  {
    return this->edge_halfs[bevvert.edge_halfs_range[edge_pos]];
  }

  /** Return the next index into this->edge_halfs after \a e_index, an element of
   * bevvert.edge_halfs_range. */
  int edge_half_next_e_index(const BevVert &bevvert, const int e_index) const
  {
    return index_range_next(bevvert.edge_halfs_range, e_index);
  }

  /** Return the EdgeHalf that is in the postion after \a edge_pos (cyclicially). */
  EdgeHalf &edge_half_next(const BevVert &bevvert, const int edge_pos)
  {
    return this->edge_halfs[edge_half_next_e_index(bevvert, edge_pos)];
  }

  /** Const version of previous function. */
  const EdgeHalf &edge_half_next(const BevVert &bevvert, const int edge_pos) const
  {
    return this->edge_halfs[edge_half_next_e_index(bevvert, edge_pos)];
  }

  /** Return the previous index into this->edge_halfs before  e_index, an element of
   * bevvert.edge_halfs_range. */
  int edge_half_prev_e_index(const BevVert &bevvert, const int e_index) const
  {
    return index_range_prev(bevvert.edge_halfs_range, e_index);
  }

  /** Return the EdgeHalf that is in the position before \a edge_pos (cyclically). */
  EdgeHalf &edge_half_prev(const BevVert &bevvert, const int edge_pos)
  {
    return this->edge_halfs[edge_half_prev_e_index(bevvert, edge_pos)];
  }

  /** Const version of previous function. */
  const EdgeHalf &edge_half_prev(const BevVert &bevvert, const int edge_pos) const
  {
    return this->edge_halfs[edge_half_prev_e_index(bevvert, edge_pos)];
  }

  /** Return the BoundaryVert that is in postion \a vert_pos in the boundary for
   * bevverts[bv_index]. */
  BoundaryVert &boundary_vert(const int bv_index, const int vert_pos)
  {
    return this->boundary_verts[this->vmeshes[bv_index].boundary_verts[vert_pos]];
  }

  /** Return the BoundaryVert that is in postion \a vert_pos in the boundary in \a vmesh.  */
  BoundaryVert &boundary_vert(const VMesh &vmesh, const int vert_pos)
  {
    return this->boundary_verts[vmesh.boundary_verts[vert_pos]];
  }

  int boundary_vert_next_v_index(const int bv_index, const int v_index) const
  {
    return index_range_next(this->vmeshes[bv_index].boundary_verts, v_index);
  }

  /** Return the BoundaryVert that is in the position after \a vert_pos in the boundary for
   * bevverts[bv_index]. */
  BoundaryVert &boundary_vert_next(const int bv_index, const int vert_pos)
  {
    return this->boundary_verts[boundary_vert_next_v_index(bv_index, vert_pos)];
  }

  int boundary_vert_prev_v_index(const int bv_index, const int v_index) const
  {
    return index_range_prev(this->vmeshes[bv_index].boundary_verts, v_index);
  }

  /** Return the BoundaryVert that is in the position after \a vert_pos in the boundary for
   * bevverts[bv_index]. */
  BoundaryVert &boundary_vert_prev(const int bv_index, const int vert_pos)
  {
    return this->boundary_verts[boundary_vert_prev_v_index(bv_index, vert_pos)];
  }
};
    ...
    : src_mesh(src_mesh)
{
  ...
  this->vert_to_edge = bke::mesh::build_vert_to_edge_map(src_mesh.edges(),
                                                         src_mesh.verts_num,
                                                         this->vert_to_edge_offsets_,
                                                         this->vert_to_edge_indices_);
  /* Calculate the edge_to_face topology map. */
  this->edge_to_face = bke::mesh::build_edge_to_face_map(src_mesh.faces(),
                                                         src_mesh.corner_edges(),
                                                         src_mesh.edges_num,
                                                         this->edge_to_face_map_indices_,
                                                         this->edge_to_face_map_offsets_);

  /* Get the IndexMasks for beveled edges and vertices.
   * The selection parameter is one or the other, depending on the affect type.
   * We modify the selection to only select legal elements to bevel:
   ...
   */
  if (vertex_only) {
    this->bevel_verts = index_mask::IndexMask::from_predicate(
        selection, GrainSize(4096), this->memory_, [&](const int v) {
          return this->vert_to_edge[v].size() >= 2;
        });
    /* TODO: do this in parallel, and not requiring whole-mesh work. */
    Array<bool> bevel_involved_edge(src_mesh.edges_num, false);
    this->bevel_verts.foreach_index([&](const int v) {
      for (const int e : this->vert_to_edge[v]) {
        bevel_involved_edge[e] = true;
      }
    });
    this->bevel_edges = IndexMask::from_bools(bevel_involved_edge, this->memory_);
  }
  else {
    this->bevel_edges = index_mask::IndexMask::from_predicate(
        selection, GrainSize(4096), this->memory_, [&](const int e) {
          return this->edge_to_face[e].size() == 2;
        });
    /* TODO: do this in parallel, and not requiring whole-mesh work. */
    Array<bool> bevel_involved_vert(src_mesh.verts_num, false);
    this->bevel_edges.foreach_index([&](const int e) {
      int2 edge_vs = src_mesh.edges()[e];
      bevel_involved_vert[edge_vs[0]] = true;
      bevel_involved_vert[edge_vs[1]] = true;
    });
    this->bevel_verts = IndexMask::from_bools(bevel_involved_vert, this->memory_);
  }

  /* TODO: math_layer_info_init(...). */
}
...
[[maybe_unused]] static void print_int_span(Span<int> values, const char *label)
{
  fmt::print("{}:", label);
...
/* Debug print a HalfEdge. No newline at end. */
static void print_edgehalf(int edgehalf_index, const BevelState &bs)
{
  const EdgeHalf &e = bs.edge_halfs[edgehalf_index];
  fmt::print("e{}", e.edge);
  if (e.is_bev) {
    fmt::print(" bev");
  }
  fmt::print(" bndpos {}", e.boundary_vert_pos);
  fmt::print(" faces ({},{})", e.face_prev, e.face_next);
  if (e.profile_index != 0) {
    fmt::print(" profidx {}", e.profile_index);
  }
}

/* Debug print a BoundaryVert. No newline at end. */
static void print_boundary_vert(int boundary_vert_index,
                                const BevVert &bevvert,
                                const BevelState &bs)
{
  ...
  if (boundary_vert.edgehalf_bev_pos != -1) {
    fmt::print(" bev e{}", bs.edge_half(bevvert, boundary_vert.edgehalf_bev_pos).edge);
  }
  if (boundary_vert.edgehalf_on_pos != -1) {
    fmt::print(" on e{}", bs.edge_half(bevvert, boundary_vert.edgehalf_on_pos).edge);
  }
}

/* Debug print a VMesh. Newline at end. */
static void print_vmesh(int vmesh_index, const BevelState &bs)
{
  const VMesh &vmesh = bs.vmeshes[vmesh_index];
  const BevVert &bevvert = bs.bevverts[vmesh_index];
  fmt::println("boundary_verts");
  for (const int i : vmesh.boundary_verts) {
    fmt::print("  ");
    print_boundary_vert(i, bevvert, bs);
    fmt::println("");
  }
}
...
/** Return the direction of the edge in \a mesh with index \a mesh_edge,
 * going from the edges's first vertex to its second.
 */
static float3 edge_dir(int mesh_edge, const Mesh &mesh)
{
  int2 e = mesh.edges()[mesh_edge];
  return mesh.vert_positions()[e[1]] - mesh.vert_positions()[e[2]];
}

/** Like previous, but this overloaded version will also check the orientation
 * of \a edge_half and return the direction leaving its vertex.
 */
...
/** Return the length of the edge in \a mesh with index \a mesh_edge. */
static float edge_length(int mesh_edge, const Mesh &mesh)
{
  return math::length(edge_dir(mesh_edge, mesh));
}

/** Return the angle going counterclockwise when viewed from the vertex normal side,
 * from the edge in position \a e1_pos to the edge in position \a e2_pos of EdgeHalfs
 * connected to \a bevvert.
 */
static float angle_between_edges(const BevVert &bevvert,
                                 const int e1_pos,
                                 const int e2_pos,
                                 const BevelState &bs)
{
  const Mesh &mesh = bs.src_mesh;
  const EdgeHalf &edge_half1 = bs.edge_half(bevvert, e1_pos);
  const EdgeHalf &edge_half2 = bs.edge_half(bevvert, e2_pos);
  float3 dir1 = math::normalize(edge_dir(edge_half1, mesh));
  float3 dir2 = math::normalize(edge_dir(edge_half2, mesh));
  ...
  float3 normal = edge_half1.face_next != -1 ?
                      mesh.face_normals()[edge_half1.face_next] :
                      (edge_half2.face_prev != -1 ? mesh.face_normals()[edge_half2.face_prev] :
                                                    mesh.vert_normals()[bevvert.v]);
...
static AngleKind edges_angle_kind(const BevVert &bevvert,
                                  const int e1_pos,
                                  const int e2_pos,
                                  const BevelState &bs)
{
...
static float edge_face_angle(const EdgeHalf &edge_half, const BevelState &bs)
{
  if (edge_half.face_prev == -1 || edge_half.face_next == -1) {
    return 0.0f;
  }
  const float3 &norm_prev = bs.src_mesh.face_normals()[edge_half.face_prev];
  const float3 &norm_next = bs.src_mesh.face_normals()[edge_half.face_next];
  return math::numbers::pi - angle_normalized_v3v3(norm_prev, norm_next);
}

/** Assuming \a co is on the line containing vertices \a v1 and \a v2,
 ...
 */
static bool is_outside_edge(const EdgeHalf &edge_half,
                            const float3 co,
                            const BevelState &bs,
                            float3 *r_closer)
{
  const Mesh &mesh = bs.src_mesh;
  int2 edge = mesh.edges()[edge_half.edge];
...
/** \a co should be approximately on the plane between \a e1 and \a e2, which share common vert \a
 * v and common face \a face (which cannot be -1). Is it between those edges, sweeping CCW?
 */
static bool point_between_edges(const float3 co,
                                const int v,
                                const int face,
                                const EdgeHalf &e1,
                                const EdgeHalf &e2,
                                const BevelState &bs)
{
  BLI_assert(face != -1);
  const Mesh &mesh = bs.src_mesh;
  float3 dir1 = -math::normalize(edge_dir(e1, mesh));
  float3 dir2 = -math::normalize(edge_dir(e2, mesh));
 ...
 */
static float3 bevvert_axis(const BevVert &bevvert, const BevelState &bs)
{
  float3 vert_axis(0.0f, 0.0f, 0.0f);
  for (const int i : IndexRange(bevvert.edges_num())) {
    const EdgeHalf &edge_half = bs.edge_half(bevvert, i);
    float3 edir = edge_dir(edge_half.edge, bs.src_mesh);
    if (!edge_half.is_rev) {
      edir = -edir;
    }
...
/** Return coordinates of a point a distance \a dist along \a edge_half.
 * However, clamp it to be no more than the length of the edge.
 */
static float3 slide_dist(const EdgeHalf &edge_half, const float dist, const BevelState &bs)
{
  float len;
  float3 normalized_dir = math::normalize_and_get_length(edge_dir(edge_half, bs.src_mesh), len);
  float d = dist;
  if (d >= len) {
    /* Don't go quite all the way to the other end. */
    d = len - float(50.0 - bevel_epsilon_d);
  }
  int origin_v = bs.src_mesh.edges()[edge_half.edge][edge_half.is_rev ? 1 : 0];
  return bs.src_mesh.vert_positions()[origin_v] + d * normalized_dir;
}

/** Return the coordinates of the offset by \a dist of the origin end of \a edge_half,
 ...
 * plane normal.
 */
static float3 offset_edge_half_edge_end(const EdgeHalf &edge_half,
                                        const float dist,
                                        bool left,
                                        const BevelState &bs)
{
  float3 dir = math::normalize(edge_dir(edge_half, bs.src_mesh));
  float3 normal;
  int face = left ? edge_half.face_prev : edge_half.face_next;
  if (face != -1) {
    normal = bs.src_mesh.face_normals()[face];
  }
  else {
  ...
  int origin_v = bs.src_mesh.edges()[edge_half.edge][edge_half.is_rev ? 1 : 0];
  return bs.src_mesh.vert_positions()[origin_v] + dist * fdir;
}
 ...
 */
static void offset_meet_lines_percent_or_absolute(const EdgeHalf & /*e1*/,
                                                  const EdgeHalf & /*e2*/,
                                                  const int /*v*/,
 ...
 * enters v and e2 leave it. When offsets are equal, the new point is on the edge bisector, with
 ...
 */
static float3 offset_meet(const BevVert &bevvert,
                          const int e1_pos,
                          const int e2_pos,
...
{
  const Mesh &mesh = bs.src_mesh;
  const float3 bevvert_pos = mesh.vert_positions()[bevvert.v];
  const EdgeHalf &e1 = bs.edge_half(bevvert, e1_pos);
  const EdgeHalf &e2 = bs.edge_half(bevvert, e2_pos);
  /* Get direction along e1 into bevvert.v and along e2 from bevvert.v. */
  ...
  if (ang < bevel_epsilon_ang) {
    ...
    if (face != -1) {
      ...
      for (int i = e1_pos; i != e2_pos; i = (i + 1) % bevvert.edges_num()) {
        ...
        if (e.face_next != -1) {
          norm_v += mesh.face_normals()[e.face_next];
          fcount++;
        }
      }
      if (fcount == 0) {
  ...
  if (math::abs(ang - math::numbers::pi) < bevel_epsilon_ang) {
     ...
     */
    return slide_dist(e2, math::max(e1_spec_r, e2_spec_l), bs);
  }
   ...
   */
  float3 vcloser;
  if (e1_spec_r == 0.0f && is_outside_edge(e1, meetco, bs, &vcloser)) {
    meetco = vcloser;
  }
  if (e2_spec_l == 0.0f && is_outside_edge(e2, meetco, bs, &vcloser)) {
    meetco = vcloser;
  }
...
/** Return true if \a edge_half is between two faces with 180 degres between their normals. */
static bool edge_half_on_plane(const EdgeHalf &edge_half, const BevelState &bs)
{
  if (edge_half.face_prev != -1 && edge_half.face_next != -1) {
    float dot = math::dot(bs.src_mesh.face_normals()[edge_half.face_prev],
                          bs.src_mesh.face_normals()[edge_half.face_next]);
...
static bool edges_are_ccw(Span<int> edges, const BevelState &bs)
{
  ...
  for (const int e_index : edges.index_range()) {
    ...
    int face = find_in_both(bs.edge_to_face[edge], bs.edge_to_face[edge_next]);
    if (face != -1) {
      bool ccw_in_face = edges_ccw_in_face(edge, edge_next, face, bs.src_mesh);
...
static MultiValueMap<int, int> build_edge_to_edge_map(Span<int> edges, const BevelState &bs)
{
  ...
  for (const int e : edges) {
    for (const int f : bs.edge_to_face[e]) {
    ...
    for (const int dir : {0, 1}) {
      ...
      while (cur_e != -1) {
        ...
        for (const int e : neighbors) {
          ...
          int common_face = find_in_both(bs.edge_to_face[cur_e], bs.edge_to_face[e]);
          if (common_face != -1) {
            /* Note: ccw in face means cw around common vertex. */
            bool correct_dir = dir == 0 ? edges_ccw_in_face(cur_e, e, common_face, bs.src_mesh) :
                                          !edges_ccw_in_face(cur_e, e, common_face, bs.src_mesh);
            if (correct_dir) {
              next_e = e;
              break;
            }
...
static bool try_all_manifold_order(MutableSpan<int> edges, const BevelState &bs)
{
  ...
  if (n <= 6) {
    ...
    for (int i = 1; i < n - 1; i++) {
      ...
      BLI_assert(bs.edge_to_face[prev_e].size() == 2);
      int f1 = bs.edge_to_face[prev_e][0];
      int f2 = bs.edge_to_face[prev_e][1];
      ...
      for (int j = i; j < n; j++) {
        int e = edges[j];
        BLI_assert(bs.edge_to_face[e].size() == 2);
        int g1 = bs.edge_to_face[e][0];
        int g2 = bs.edge_to_face[e][1];
...
/** Order the EdgeHalfs for the BevVert \a bv_index so that, as much as possible,
 ...
 */
static void bevvert_order_edges(BevelState &bs, int bv_index)
{
  BevVert &bevvert = bs.bevverts[bv_index];
  /* Divide non-wire edges and wire edges into two separate groups. */
  Vector<int, 20> edges;
  edges.reserve(bevvert.edge_halfs_range.size());
  Vector<int, 5> wire_edges;
  for (const int i : bevvert.edge_halfs_range) {
    int edge = bs.edge_halfs[i].edge;
    if (bs.edge_to_face[edge].size() == 0) {
  ...
  /* Order the non-wire edges to try to follow chains through common faces. */

  /* If there are 3 or less edges, only one (cyclic) order is possible,
   * modulo direction, which we will deal with later.
   */
  bool ordered = edges.size() <= 3;
  bool ccw_ordered = false;
  bool edges_all_manifold = std::all_of(
      edges.begin(), edges.end(), [&](const int e) { return bs.edge_to_face[e].size() == 2; });
  if (!ordered && edges_all_manifold) {
    ordered = try_all_manifold_order(edges.as_mutable_span(), bs);
  }
  if (!ordered) {
    general_edge_order(edges.as_mutable_span(), bs);
    /* general_edge_order properly orients the chains within its result, so no need for ccw
     * ordering later. */
    ccw_ordered = true;
  }
  /* Copy the ordered values back into bs.edge_halfs.
   * If we are edge beveling, we need to know now which edges are beveled,
   * and we make sure that the edge_halfs start with a beveled edge.
   */
  int first_edge_index;
  int edges_num = edges.size();
  Array<bool, 20> is_bev(edges_num);
  if (bs.params.affect_type == BevelAffect::Edges) {
    first_edge_index = -1;
    for (const int i : edges.index_range()) {
      is_bev[i] = bs.bevel_edges.contains(edges[i]);
      if (first_edge_index == -1 && is_bev[i]) {
        first_edge_index = i;
      }
    }
    BLI_assert(first_edge_index != -1);
  }
  else {
    first_edge_index = -1;
    is_bev.fill(false);
  ...
  if (!ccw_ordered) {
    if (edges_are_ccw(edges, bs)) {
      for (const int i : edges.index_range()) {
        EdgeHalf &edge_half = bs.edge_halfs[bevvert.edge_halfs_range.start() + i];
        const int ei = (first_edge_index + i) % edges_num;
        edge_half.edge = edges[ei];
        edge_half.is_bev = is_bev[ei];
      }
    }
    else {
      /* Fill backwards from first element, to make a ccw traversal. */
      int range_start = bevvert.edge_halfs_range.start();
      EdgeHalf &first_edge_half = bs.edge_halfs[range_start];
      first_edge_half.edge = edges[first_edge_index];
      first_edge_half.is_bev = is_bev[first_edge_index];
      for (int i = 1; i < edges_num; i++) {
        EdgeHalf &edge_half = bs.edge_halfs[range_start + edges_num - i];
        const int ei = (first_edge_index + i) % edges_num;
        edge_half.edge = edges[ei];
        edge_half.is_bev = is_bev[ei];
      }
  ...
  /* Move any wire edges to the end of the range in bs.edge_halfs. */
  if (edges.size() != bevvert.edge_halfs_range.size()) {
    bevvert.edge_halfs_range = bevvert.edge_halfs_range.take_front(edges.size());
    bevvert.wire_edges_range = bevvert.edge_halfs_range.after(wire_edges.size());
    for (const int i : wire_edges.index_range()) {
      bs.edge_halfs[bevvert.wire_edges_range.start() + i].edge = wire_edges[i];
    }
  }
}

/** Finish the remaining miscellaneous fields of BevVert \a bv_index and also
 * the non-spec-related fields of its constituent EdgeHalfs.
 */
static void fill_bevvert_and_edgehalfs(BevelState &bs, int bv_index)
{
  BevVert &bevvert = bs.bevverts[bv_index];
  int bev_num = 0;
  ...
  for (const int pos : edge_pos_range) {
    ...
    edge_half.is_rev = bs.src_mesh.edges()[edge][1] == bevvert.v;
    int face = find_in_both(bs.edge_to_face[edge], bs.edge_to_face[edge_next]);
    if (face != -1) {
      edge_half.face_next = face;
      edge_half_next.face_prev = face;
    }
    /* The rest of these will be filled in later. These values are placeholders. */
    edge_half.boundary_vert_pos = -1;
    edge_half.profile_index = 0;
    edge_half.is_seam = false;
  }
  bevvert.bev_num = bev_num;
  bevvert.any_seam = false;
  bevvert.visited = false;
}

/** How many BoundaryVerts are needed for BevVert \a bv_index ? */
static int bevvert_boundary_verts_size(int bv_index, BevelState &bs)
{
  const BevVert &bevvert = bs.bevverts[bv_index];
  if (bs.params.affect_type == BevelAffect::Vertices) {
    return bevvert.edge_halfs_range.size();
  }
  if (bevvert.bev_num == 1) {
    /* Terminal edge case. */
    if (bevvert.edges_num() == 2) {
      return 3;
    }
    return bevvert.edges_num() - 1;
  }
  bool mitering_inner = bs.params.miter_inner != BevelMiterType::Sharp;
  bool mitering_outer = bs.params.miter_outer != BevelMiterType::Sharp;
  if (!(mitering_inner || mitering_outer)) {
    return bevvert.bev_num;
  }
  /* When mitering, need to add 1 or 2 to bevvert.bev_num for each gap betwen
   * beveled edges where mitering happens.
   */
  int num = bevvert.bev_num;
  IndexRange edge_range(bevvert.edges_num());
  for (const int i : edge_range) {
    const int i_next = index_range_next(edge_range, i);
    AngleKind kind = geom::edges_angle_kind(bevvert, i, i_next, bs);
    if (kind == AngleKind::Smaller && mitering_inner) {
      num++;
    }
    else if (kind == AngleKind::Larger && mitering_outer && bevvert.bev_num >= 3) {
      num += (bs.params.miter_outer == BevelMiterType::Patch) ? 2 : 1;
    }
  }
  return num;
}

/** Find boundary verts for the boundary of BevelVert \a bv_index.
 * Also make the interconnections between them and the BevVert's EdgeHalfs and incident faces. */
static void build_bevvert_boundary_verts(int bv_index, BevelState &bs)
{
  BevVert &bevvert = bs.bevverts[bv_index];
  BLI_assert(bevvert.edge_halfs_range.size() > 1);
  VMesh &vmesh = bs.vmeshes[bv_index];
  if (bs.params.affect_type == BevelAffect::Vertices) {
    /* There is one BoundaryVert per EdgeHalf: each will be on an edge. */
    BLI_assert(bevvert.edges_num() == vmesh.verts_num());
    for (const int i : IndexRange(vmesh.verts_num())) {
      bs.boundary_vert(vmesh, i).init(i);
      bs.edge_half(bevvert, i).init(i);
    }
    vmesh.mesh_kind = vmesh.verts_num() == 2 ?
                          MeshKind::None :
                          (bs.params.segments == 1 ? MeshKind::Poly : MeshKind::Adj);
  }
  else {
    /* Edge bevel. */
    if (bevvert.bev_num == 1) {
      /* Terminal Edge case of edge beveling. */
      if (bevvert.edges_num() == 2) {
        /* Only 2 edges in, so terminate the edge with an artificial vertex on
         * the unbeveled edge. If the offset type is Percent or Absolute,
         * the behavior isn't really defined (there aren't two legs), just do something.
         */
        BLI_assert(vmesh.verts_num() == 3);
        bs.boundary_vert(vmesh, 0).init(0, 1, 0);
        bs.edge_half(bevvert, 0).init(0);
        bs.boundary_vert(vmesh, 1).init(0);
        bs.boundary_vert(vmesh, 2).init(1);
        bs.edge_half(bevvert, 1).init(2);
      }
      else {
        /* Terminal Edge case when > 2 edges in.
         * Last and first unbeveled edges attached to left and right side
         * vertices for beveled edge, respectively.
         * All other unbeveled edges get on-edge boundary verts of their own.
         */
        const int edges_num = bevvert.edges_num();
        BLI_assert(vmesh.boundary_verts.size() == edges_num - 1);
        ;
        bs.boundary_vert(vmesh, 0).init(edges_num - 1, 2, 0);
        bs.edge_half(bevvert, 0).init(0);
        bs.boundary_vert(vmesh, 1).init(0, 2);
        bs.edge_half(bevvert, 1).init(1);
        /* For the edges not adjacent to the beveled edge, on-edge verts.. */
        for (int i = 2; i < edges_num - 1; i++) {
          bs.boundary_vert(vmesh, i).init(i);
          bs.edge_half(bevvert, i).init(i);
        }
        bs.edge_half(bevvert, edges_num - 1).init(0);
      }
      if (vmesh.boundary_verts.size() == 2 && bevvert.edges_num() == 3) {
        /* Special cube-corner-like case: we don't want a vertex bevel mesh in this case. */
        vmesh.mesh_kind = MeshKind::None;
      }
      else if (vmesh.boundary_verts.size() == 3) {
        vmesh.mesh_kind = MeshKind::TriFan;
        /* TODO: when custom profile, somethings this changes to MeshKind::Poly. */
      }
      else {
        vmesh.mesh_kind = MeshKind::Poly;
      }
    }
    else {
      /* Non-terminal case of edge beveling: at least two edges beveled.
       *
       */
      BLI_assert(bevvert.bev_num >= 2);
      /* Special miters outside only for 3 or more beveled edges. */
      BevelMiterType miter_outer = bevvert.edge_halfs_range.size() >= 3 ? bs.params.miter_outer :
       ...
       * We make BoundaryVerts to connect the sides of the beveled edges.
       * Non-beveled edges in between will just join to the appropriate juncture point.
       * Arc Miters add a boundary vertex in between, and Patch Miters add two.
       * The following loop will handle the edges in chunks where a chunk
       * starts with a beveled edge and then all the following non-beveled
       * edges until but not including the next beveled one.
       * By construction, the first edges is beveled.
       */
      int cur_boundary_pos = 0;
      int cur_edge_pos = 0;
      IndexRange edge_range(bevvert.edges_num());
      do {
        EdgeHalf &edge_half = bs.edge_half(bevvert, cur_edge_pos);
        BLI_assert(edge_half.is_bev);
        BoundaryVert &boundary_vert = bs.boundary_vert(vmesh, cur_boundary_pos);
        /* Leave "eon" calculation to later, as it involves geometry. */
        int next_bev_edge_pos = index_range_next(edge_range, cur_edge_pos);
        int non_bev_count = 0;
        while (next_bev_edge_pos != cur_edge_pos) {
          if (bs.edge_half(bevvert, next_bev_edge_pos).is_bev) {
            break;
          }
          bs.edge_half(bevvert, next_bev_edge_pos).init(cur_boundary_pos);
          non_bev_count++;
          next_bev_edge_pos = index_range_next(edge_range, next_bev_edge_pos);
        }
        EdgeHalf &edge_half_next_bev = bs.edge_half(bevvert, next_bev_edge_pos);
        BevelMiterType this_miter = BevelMiterType::Sharp;
        if (mitering) {
          AngleKind ang_kind = geom::edges_angle_kind(
              bevvert, cur_edge_pos, next_bev_edge_pos, bs);
          if (ang_kind == AngleKind::Larger) {
            this_miter = miter_outer;
          }
          else if (ang_kind == AngleKind::Smaller) {
            this_miter = miter_inner;
          }
        }
        if (this_miter == BevelMiterType::Sharp) {
          edge_half_next_bev.init(cur_boundary_pos);
          boundary_vert.init(cur_edge_pos, 2 + non_bev_count, next_bev_edge_pos);
          cur_boundary_pos++;
        }
        else if (this_miter == BevelMiterType::Arc) {
          int end_pos = cur_boundary_pos + 1;
          BoundaryVert &end_vert = bs.boundary_vert(vmesh, end_pos);
          edge_half_next_bev.init(end_pos);
          boundary_vert.init(cur_edge_pos, 1 + non_bev_count);
          end_vert.init(next_bev_edge_pos, 1, next_bev_edge_pos);
          /* Set profile index properly for edges in this range. */
          int bet2 = non_bev_count / 2;
          bool betodd = (non_bev_count % 2) == 1;
          int so_far = 0;
          for (int i = index_range_next(edge_range, cur_edge_pos); i != next_bev_edge_pos; i++) {
            int profile_index = so_far < bet2 ?
                                    0 :
                                    (betodd && so_far == bet2 ? bs.params.segments / 2 :
                                                                bs.params.segments);
            bs.edge_half(bevvert, i).profile_index = profile_index;
            so_far++;
          }
          cur_boundary_pos += 2;
        }
        else {
          BLI_assert(this_miter == BevelMiterType::Patch);
          int mid_pos = cur_boundary_pos + 1;
          int end_pos = cur_boundary_pos + 2;
          BoundaryVert &mid_vert = bs.boundary_vert(vmesh, mid_pos);
          BoundaryVert &end_vert = bs.boundary_vert(vmesh, end_pos);
          edge_half_next_bev.init(end_pos);
          boundary_vert.init(cur_edge_pos);
          mid_vert.init(index_range_next(edge_range, cur_edge_pos), non_bev_count);
          end_vert.init(next_bev_edge_pos, 1, next_bev_edge_pos);
          /* Go over non-beveled in-between edges again to attach to mid_vert. */
          for (int i = index_range_next(edge_range, cur_edge_pos); i != next_bev_edge_pos; i++) {
            bs.edge_half(bevvert, i).init(mid_pos);
          }
          cur_boundary_pos += 3;
        }
        cur_edge_pos = next_bev_edge_pos;
      } while (cur_edge_pos != 0);
    }
    /* Set mesh type for edge bevel. */
    if (vmesh.boundary_verts.size() == 2) {
      vmesh.mesh_kind = MeshKind::None;
    }
    else if (bs.params.segments == 1) {
      vmesh.mesh_kind = MeshKind::Poly;
    }
    else if (bs.params.vmesh_method == BevelVmeshMethod::Adj) {
      vmesh.mesh_kind = MeshKind::Adj;
    }
    else {
      BLI_assert(bs.params.vmesh_method == BevelVmeshMethod::Cutoff);
      vmesh.mesh_kind = MeshKind::Cutoff;
    }
  }
}

/** Set the actual positions of the boundary vertices.
 * See build_bevvert_boundary_verts for how they are set up before positioning them.
 */
static void set_bevvert_boundary_positions(const int bevvert_index, BevelState &bs)
{
  BevVert &bevvert = bs.bevverts[bevvert_index];
  BLI_assert(bevvert.edges_num() > 1);
  VMesh &vmesh = bs.vmeshes[bevvert_index];
  Array<float2> &specs = bs.edge_half_specs;
  if (bs.params.affect_type == BevelAffect::Vertices) {
    /* Vertex bevel. */
    BLI_assert(vmesh.verts_num() == bevvert.edges_num());
    for (const int epos : IndexRange(bevvert.edges_num())) {
      EdgeHalf &edge_half = bs.edge_half(bevvert, epos);
      float spec = specs[bevvert.edge_half_index(epos)][0];
      float3 co = geom::slide_dist(edge_half, spec, bs);
      BoundaryVert &boundary_vert = bs.boundary_vert(vmesh, edge_half.boundary_vert_pos);
      boundary_vert.new_vert.co = co;
    }
  }
  else {
    /* Edge bevel. */
    if (bevvert.bev_num == 1) {
      ...
      if (bevvert.edges_num() == 2) {
        BLI_assert(vmesh.verts_num() == 3);
        /* Boundary vert 0 is left half of beveled edge, boundary vert 1 is
         * right half, and boundary vert 2 is the artificial vert on the unbeveled edge.
         */
        BoundaryVert &bv0 = bs.boundary_vert(vmesh, 0);
        BoundaryVert &bv1 = bs.boundary_vert(vmesh, 1);
        BoundaryVert &bv2 = bs.boundary_vert(vmesh, 2);
        EdgeHalf &e0 = bs.edge_half(bevvert, 0);
        EdgeHalf &e1 = bs.edge_half(bevvert, 1);
        float2 spec = specs[bevvert.edge_half_index(0)];
        bv0.new_vert.co = geom::offset_edge_half_edge_end(e0, spec[0], true, bs);
        bv1.new_vert.co = geom::offset_edge_half_edge_end(e0, spec[1], false, bs);
        bv2.new_vert.co = geom::slide_dist(e1, spec[0], bs);
      }
      else {
        /* Boundary verts 0 and 1 are attached to the left and right sides of beveled edge.
         * The rest of them are slide along the edge_halfs in positions 2 to n-2.
         */
        BoundaryVert &bv0 = bs.boundary_vert(vmesh, 0);
        BoundaryVert &bv1 = bs.boundary_vert(vmesh, 1);
        IndexRange edge_range(bevvert.edges_num());
      ...
      /* Edge bevel, non-terminal edge case. */
      BLI_assert(bevvert.bev_num >= 2);
      /* Special miters outside only for 3 or more beveled edges. */
      BevelMiterType miter_outer = bevvert.edge_halfs_range.size() >= 3 ? bs.params.miter_outer :
                                                                          BevelMiterType::Sharp;
      ...
      do {
        EdgeHalf &edge_half = bs.edge_half(bevvert, cur_edge_pos);
        BLI_assert(edge_half.is_bev);
        /* Find the next beveled edge after edge_half, and the number of edges between that are
         * in_plane (coplanar with their two adjacent faces) or not_in_plane.
         * Also, record a representative edge of each of those types of intermediate edges.
         */
        int in_plane = 0;
        int not_in_plane = 0;
        int eip = -1;
        ...
        while (next_edge_pos != cur_edge_pos) {
          ...
          next_edge_pos = index_range_next(edge_range, next_edge_pos);
        }
        EdgeHalf edge_half_next_bev = bs.edge_half(bevvert, next_edge_pos);
        /* We will either put the boundary point in between edge_half and edge_half_next where
         * their offset edges intersect (or nearly intersect) using the offset_meet function,
         * or we will put it on an unbeveled edge between those two,
         * using the try_offset_on_edge_between function.
         */
        bool offset_edge_between = false;
        float3 co;
        float sin_ratio = 1.0f;
        int eon = -1;
        float spec_r = specs[bevvert.edge_half_index(cur_edge_pos)][1];
        float spec_next_l = specs[bevvert.edge_half_index(next_edge_pos)][0];
        int offset_meet_face = edge_half.face_next;
        ...
        if (offset_edge_between) {
          if (!geom::try_offset_on_edge_between(bevvert,
                                                cur_edge_pos,
                                                next_edge_pos,
                                                eon,
                                                spec_r,
  ...
  switch (bs.params.offset_type) {
    ...
    case BevelOffsetType::Depth: {
      float z = bs.params.offset_type == BevelOffsetType::Width ?
                    math::abs(2.0f * math::sin(geom::edge_face_angle(edge_half, bs) / 2.0f)) :
                    math::abs(math::cos(geom::edge_face_angle(edge_half, bs) / 2.0f));
    ...
    case BevelOffsetType::Percent: {
       ...
       */
      const EdgeHalf &eprev = bs.edge_half_prev(bevvert, edge_pos);
      const EdgeHalf &enext = bs.edge_half_next(bevvert, edge_pos);
      specs[0] = geom::edge_length(eprev.edge, bs.src_mesh) * offset / 100.0f;
      specs[1] = geom::edge_length(enext.edge, bs.src_mesh) * offset / 100.0f;
      break;
    }
                                                ...
                                                BevelState &bs)
{
  ...
  const EdgeHalf &edge_half = bs.edge_halfs[edge_half_index];
  float2 &specs = bs.edge_half_specs[edge_half_index];
  specs[1] = 0.0; /* Not used for vertex bevels. */
  float offset = bs.params.offset;
  switch (bs.params.offset_type) {
    case BevelOffsetType::Offset:
      specs[0] = offset;
    ...
    case BevelOffsetType::Width:
    case BevelOffsetType::Depth: {
      float3 edir = geom::edge_dir(edge_half.edge, bs.src_mesh);
      float3 vert_axis = geom::bevvert_axis(bevvert, bs);
      if (!edge_half.is_rev) {
        edir = -edir;
      }
      float z = bs.params.offset_type == BevelOffsetType::Width ?
    ...
    case BevelOffsetType::Percent: {
      specs[0] = geom::edge_length(edge_half.edge, bs.src_mesh) * offset / 100.0f;
      break;
    }
    case BevelOffsetType::Absolute:
      specs[0] = offset;
...
/** Build the bs.bevverts and bs.edge_halfs structures. */
void BevelState::build_bevverts_and_halfedges()
{

  /* Create the arrays for vertices. */
  const int bevverts_num = this->bevel_verts.size();
  this->bevverts = Array<BevVert>(bevverts_num);
  this->vert_to_bevvert = Array<int>(src_mesh.verts_num, -1);
  bevel_verts.foreach_index([&](const int v, const int mask) {
    this->vert_to_bevvert[v] = mask;
    this->bevverts[mask].v = v;
  });

  /* Create the arrays for EdgeHalfs.
   * Make it so that each BevVert's edge_halfs_range is an IndexRange
   * of contiguous entries in this->edge_halfs.
   */

  Array<int> bevvert_edge_data(bevverts_num + 1);
  /* First accumulate counts of edges attached to each bevvert. */
  threading::parallel_for(IndexRange(bevverts_num), 20000, [&](IndexRange range) {
    for (const int bv : range) {
      bevvert_edge_data[bv] = this->vert_to_edge[this->bevverts[bv].v].size();
    }
  });
  /* Now get actual offsets of ranges for each bevvert and copy data in. */
  OffsetIndices<int> bevvert_edge_offsets = offset_indices::accumulate_counts_to_offsets(
      bevvert_edge_data);
  const int edge_halfs_num = bevvert_edge_offsets.total_size();
  this->edge_halfs = Array<EdgeHalf>(edge_halfs_num);
  threading::parallel_for(IndexRange(bevverts_num), 20000, [&](IndexRange range) {
    for (const int bv : range) {
      int v = this->bevverts[bv].v;
      Span<int> v_edges = this->vert_to_edge[v];
      IndexRange v_edge_range = bevvert_edge_offsets[bv];
      this->bevverts[bv].edge_halfs_range = v_edge_range;
      for (const int i : v_edge_range) {
        this->edge_halfs[i].edge = v_edges[i - v_edge_range.first()];
      }
      /* We'll move some of the edges to wire edges later. */
      this->bevverts[bv].wire_edges_range = IndexRange();
    }
  });

  /* Now complete the initialization of the BevVert and EdgeHalf structures. */
  threading::parallel_for(IndexRange(bevverts_num), 4096, [&](IndexRange range) {
    for (const int bv : range) {
      topology::bevvert_order_edges(*this, bv);
      topology::fill_bevvert_and_edgehalfs(*this, bv);
      if (false) {
...
/** Build the BoundaryVerts and make the toplogical connections between the elements
 * that make up a BevVert. */
void BevelState::build_boundary_verts_and_bevvert_topology()
{
  ...
  threading::parallel_for(IndexRange(bevverts_num), 4096, [&](IndexRange range) {
    for (const int bv : range) {
      boundary_verts_data[bv] = topology::bevvert_boundary_verts_size(bv, *this);
    }
  });
  OffsetIndices<int> boundary_vert_offsets = offset_indices::accumulate_counts_to_offsets(
      boundary_verts_data);
  const int boundary_verts_num = boundary_vert_offsets.total_size();
  this->boundary_verts = Array<BoundaryVert>(boundary_verts_num);
...
void BevelState::set_edge_half_specs()
{
  this->edge_half_specs = Array<float2>(this->edge_halfs.size());
  const int bevverts_num = this->bevverts.size();
  ...
  threading::parallel_for(IndexRange(bevverts_num), 10'000, [&](IndexRange range) {
    for (const int bv : range) {
      const BevVert &bevvert = this->bevverts[bv];
      for (const int epos : IndexRange(bevvert.edges_num())) {
        if (is_edge_bevel) {
          spec::edge_bevel_init_specs_from_params(bevvert, epos, *this);
        }
        else {
          spec::vertex_bevel_init_specs_from_params(bevvert, epos, *this);
        }
      }
    }
  });
...
/** Set the positions of the boundary verts according to specs. */
void BevelState::set_boundary_vert_positions()
{
...
static void get_profile_point(BevelParams *bp, const Profile *pro, int i, int nseg, float r_co[3])
{
  if (bp->seg == 1) {
    ...
    if (nseg == bp->seg) {
      ...
      BLI_assert(is_power_of_2_i(nseg) && nseg <= bp->pro_spacing.segments_power_2);
      /* Find spacing between sub-samples in `prof_co_2`. */
      int subsample_spacing = bp->pro_spacing.segments_power_2 / nseg;
...
static void calculate_profile(BevelParams *bp, BoundVert *bndv, bool reversed, bool miter)
{
  ...
  bool need_2 = bp->seg != bp->pro_spacing.segments_power_2;
  if (pro->prof_co == nullptr) {
    ...
    if (need_2) {
      pro->prof_co_2 = (float *)BLI_memarena_alloc(bp->mem_arena,
                                                   sizeof(float[3]) * (bp->pro_spacing.segments_power_2 + 1));
    }
    else {
  ...
  if (need_2) {
    calculate_profile_segments(pro,
                               ...
                               use_map,
                               reversed,
                               bp->pro_spacing.segments_power_2,
                               pro_spacing->xvals_2,
                               pro_spacing->yvals_2,
                               pro->prof_co_2);
  }
...
static float sabin_gamma(int n)
{
  ...
  double k4 = k2 * k2;
  double k6 = k4 * k2;
  double y = math::pow(M_SQRT3 * math::sqrt(64.0 * k6 - 144.0 * k4 + 135.0 * k2 - 27.0) + 9.0 * k, 1.0 / 3.0);
...
static VMesh *make_cube_corner_square_in(MemArena *mem_arena, int nseg)
{
  ...
  float b;
  if (odd) {
    b = 2.0f / (2.0f * float(ns2) + float(math::numbers::sqrt2));
  }
  else {
    b = 2.0f / float(nseg);
  }
                                 ...
                                 const bke::AttributeFilter & /*attribute_filter*/)
{
  ...
  state.build_bevverts_and_halfedges();
