...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  if (node) {
    ...
    for (const int i : IndexRange(storage.items_num)) {
      ...
      const UString mask_id(MaskStackItemsAccessor::socket_identifier_for_item(item));
      ...
      if (tree) {
        mask_decl.socket_name_ptr(&tree->id, *MaskStackItemsAccessor::item_srna, &item, "name");
      }
      ...
      const UString opacity_id(MaskStackItemsAccessor::opacity_socket_identifier_for_item(item));
  ...
  b.add_input<decl::Extend>(""_ustr, "__extend__"_ustr)
      .custom_draw(socket_items::ui::draw_extend_socket_fn<MaskStackItemsAccessor>());
  b.add_output<decl::Float>("Result"_ustr).structure_type(StructureType::Single);
}

static void node_operators()
{
  socket_items::ops::make_common_operators<MaskStackItemsAccessor>();
}
...
static void node_layout_ex(ui::Layout &layout, bContext *C, PointerRNA *node_ptr)
{
  ...
  socket_items::ui::draw_items_list_with_operators<MaskStackItemsAccessor>(
      C, &layout, ntree, node);
  const NodeShaderLayerStack &storage = layer_stack_storage(node);
  socket_items::ui::draw_active_item_props<MaskStackItemsAccessor>(
...
void register_node_type_sh_mask_stack()
{
  ...
  using Accessor = nodes::MaskStackItemsAccessor;
...
StructRNA **MaskStackItemsAccessor::item_srna = &RNA_ShaderMaskStackItem;
