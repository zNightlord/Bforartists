    ...
    int structure_type, float x, float y, float w, float h, float /*alpha*/)
{
  ...
  float color_outer[4] = {0};
  blender::ui::theme::get_color_type_4fv(TH_WIRE, SPACE_NODE, color_outer);
  color_outer[3] = 1.0f;
                                         ...
                                         const size_t end)
{
  ...
  for (const ColorItem &item : items) {
    ...
    if (name == item.name) {
      ...
      if (name.size() > item_name_len && name.compare(0, item_name_len, item_name) == 0 &&
...
ImBuf *icon_alert_imbuf_get(AlertIcon icon, float size)
{
  ...
  return svg_icon_bitmap(icon_id, size, true); /* BFA - color dialog icon */
#endif
}
