...
static wmOperatorStatus sequencer_slip_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  WM_event_add_notifier(
      C, NC_SCENE | ND_SEQUENCER, scene); /* seq::get_ref_scene_for_notifiers(C)); /*BFA - 3D Sequencer*/
...
static void sequencer_delete_strip_data(bContext *C, Strip *strip)
{
  ...
  if (strip->scene) {
    if (ED_scene_delete(C, bmain, strip->scene)) {
      WM_event_add_notifier(C, NC_SEQUENCE | NA_REMOVED, strip->scene);
    }
  }
}
...
static wmOperatorStatus sequencer_delete_exec(bContext *C, wmOperator *op)
{
  ...
  WM_event_add_notifier(C, NC_SEQUENCE | ND_SEQUENCER, scene); /* seq::get_ref_scene_for_notifiers(C)); /*BFA - 3D Sequencer*/
  WM_event_add_notifier(C, NC_SEQUENCE | ND_ANIMCHAN, scene); /* seq::get_ref_scene_for_notifiers(C)); /*BFA - 3D Sequencer*/
  return OPERATOR_FINISHED;
}
...
static wmOperatorStatus sequencer_set_range_to_strips_exec(bContext *C, wmOperator *op)
{
  ...
  WM_event_add_notifier(C, NC_SEQUENCE | ND_FRAME_RANGE, scene);
