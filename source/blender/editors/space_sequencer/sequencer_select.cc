...
static const EnumPropertyItem sequencer_prop_select_grouped_types[] = {
    {SEQ_SELECT_GROUP_TYPE, "TYPE", ICON_TYPE, "Type", "Select all strips of the same specific type as the active strip"},
    {SEQ_SELECT_GROUP_DATA, "DATA", ICON_TEXT, "Data", "Select strips referencing the same source data as the active strip, with matching file directory, scene, clip, or mask"},
    {SEQ_SELECT_GROUP_STRIPS_UNDER_EFFECT, "STRIPS_UNDER_EFFECT", ICON_PHYSICS, "Strips Under Effect", "Select strips that are inputs to the same effect type as the active strip"},
    {SEQ_SELECT_GROUP_TIME_OVERLAP, "TIME_OVERLAP", ICON_LAYER, "Time Overlap", "Select strips on any channel whose time range overlaps the active strip"},
    {SEQ_SELECT_GROUP_VISUAL_OVERLAP, "VISUAL_OVERLAP", ICON_LINKED, "Visual Overlap", "Select strips on channels below the active strip's channel whose time range overlaps, plus their full effect chains"},
    {0, nullptr, 0, nullptr, nullptr},
};
