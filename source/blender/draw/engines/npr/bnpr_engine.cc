...
typedef struct bnpr_Data {
  ...
  npr::strokegen::Instance *instance;
...
static void bnpr_engine_init(void *vedata)
{
  ...
  if (ved->instance == nullptr) {
    ved->instance = new npr::strokegen::Instance();
  }
...
static void bnpr_cache_init_legacy(void *vedata)
{
  ...
  for (int i = 0; i < 2; i++) {
    ...
    blender::npr::strokegen::StrokeGenShaderModule* shaderModule =
      blender::npr::strokegen::StrokeGenShaderModule::module_get();

    GPUShader *sh = DRW_state_is_select() ?
                        shaderModule->static_shader_get(blender::npr::strokegen::DEPTH_CONSERVATIVE) :
                        shaderModule->static_shader_get(blender::npr::strokegen::DEPTH);
    ...
    sh = DRW_state_is_select() ?
        shaderModule->static_shader_get(blender::npr::strokegen::POINTCLOUD_DEPTH_CONSERVATIVE) :
        shaderModule->static_shader_get(blender::npr::strokegen::POINTCLOUD_DEPTH);
    ...
    stl->g_data->depth_hair_shgrp[i] = grp = DRW_shgroup_create(
        shaderModule->static_shader_get(blender::npr::strokegen::DEPTH), psl->depth_pass[i]);
    DRW_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);

    stl->g_data->depth_curves_shgrp[i] = grp = DRW_shgroup_create(
        shaderModule->static_shader_get(blender::npr::strokegen::CURVES_DEPTH), psl->depth_pass[i]);
    DRW_shgroup_uniform_block(grp, "globalsBlock", G_draw.block_ubo);

    sh = DRW_state_is_select() ? shaderModule->static_shader_get(blender::npr::strokegen::DEPTH_CONSERVATIVE) :
                                 shaderModule->static_shader_get(blender::npr::strokegen::DEPTH);
...
{
  npr::strokegen::StrokeGenShaderModule::module_free();
}
