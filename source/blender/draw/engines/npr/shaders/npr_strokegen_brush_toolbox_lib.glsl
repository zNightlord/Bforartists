...
uint get_wing_quad_vertex_index(uint drw_vtx_id)
{
...
{
    ssbo_stroke_mesh_pool_[curve_sample_id * 3 + 0] = pack_2d_uv(verts[0]); 
    ssbo_stroke_mesh_pool_[curve_sample_id * 3 + 1] = pack_2d_uv(verts[1]); 
    ssbo_stroke_mesh_pool_[curve_sample_id * 3 + 2] = pack_2d_uv(verts[2]); 
}
vec2 load_ssbo_stroke_mesh_pool__skeletal_VB(uint drw_vtx_id)
{
    wq_vtx_id = get_wing_quad_vertex_index(drw_vtx_id); 
...
{
    uint offset = num_samples * 3u + curve_sample_id * 2u;
    ssbo_stroke_mesh_pool_[offset * 2u + 0u] = pack_2d_uv(col.rg); 
    ssbo_stroke_mesh_pool_[offset * 2u + 1u] = pack_2d_uv(col.ba); 
}
vec4 load_ssbo_stroke_mesh_pool__skeletal_color(uint curve_sample_id, uint num_samples)
{
    uint offset = num_samples * 3u + curve_sample_id * 2u;
    vec4 col; 
    col.rg = unpack_2d_uv(ssbo_stroke_mesh_pool_[offset * 2u + 0u]); 
    col.ba = unpack_2d_uv(ssbo_stroke_mesh_pool_[offset * 2u + 1u]); 
    return col; 
}
